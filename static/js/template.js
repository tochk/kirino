...
$("input.mac_valid").on('input', '[data-action="text"]', function () {
  var $item = $(this),
    value = $item.val();
    console.log(CheckMACAddress(value));
});
...
$("input.mac").focusout(function(){
    ...
    if(CheckMACAddress($(this).val())==false){
            $(this).parent().parent().addClass('has-error1');
            $(this).parent().parent().removeClass('has-success');    
...
moveCaretToStart(document.querySelector('#inputPassword')); 
